...
export const MatchPage = () => {
  ...
  const audioFail = useMemo(() => createSound(failSong2, soundVolume), [
    soundVolume,
  ]);
  ...
  const answer = useCallback(
    (src) => {
      ...
      if (src === currentWord.src) {
        ...
        const goodOverlay = four.current.find(
          (elem) => elem.dataset.name === src
        );
        ...
        const goodOverlay = four.current.find(
          (elem) => elem.dataset.name === currentWord.src
        );
        const badOverlay = four.current.find(
          (elem) => elem.dataset.name === src
        );
        goodOverlay.classList.add(classes.goodOverlay);
  ...
  useEffect(() => {
    if (wordsArray.length && currentNumber < wordsArray.length) {
      const arr = wordsArray
        .filter((word) => word.src !== currentWord.src)
    ...
    <div className={classes.root}>
      {endGame ? (
        ...
        <div ref={gameBoard} className={classes.gameContainer}>
          ...
          <Typography className={classes.word} variant="h3">{`${
            currentWord.english || ''
          }`}</Typography>
