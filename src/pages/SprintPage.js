...
export const SprintPage = () => {
  ...
  const audioFail = useMemo(() => createSound(failSong, soundVolume * 5), [
    soundVolume,
  ]);
  ...
  useEffect(() => {
    ...
    const keyboardClick = (event) => {
      ...
      let value;
      if (
        event.keyCode === keyCodeArray.enter ||
        event.keyCode === keyCodeArray.num2
      ) {
        value = 'true';
      } else if (
        event.keyCode === keyCodeArray.space ||
    ...
    <div className={classes.root}>
      {endGame ? (
        ...
        <div ref={gameBoard} className={classes.gameContainer}>
          ...
          <Typography variant="h4">{`${currentWord.english || ''} = ${
            currentRussianhWord || ''
          }`}</Typography>
