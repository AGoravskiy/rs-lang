...
export const SavannaPage = () => {
  ...
  const soundVolume = useMemo(
    () =>
      localStorage.getItem(LOCAL_STORAGE_KEY.soundVolume) ||
      INIT_CONSTS.soundVolume,
    []
  );
  const musicVolume = useMemo(
    () =>
      localStorage.getItem(LOCAL_STORAGE_KEY.musicVolume) ||
  ...
  const audioSuccess = useMemo(() => createSound(successSong, soundVolume), [
    soundVolume,
  ]);
  const audioFail2 = useMemo(() => createSound(failSong2, soundVolume), [
    soundVolume,
  ]);
  ...
  const putStats = useCallback(async () => {
    try {
      ...
      const correctPercent = Math.round(
        (100 * correctAnswers.length) /
          (correctAnswers.length + failAnswers.length)
      );
    ...
    (value, click) => {
      ...
      if (click) {
        if (value === currentWord.english) {
          ...
          audioSuccess.play();
          const goodButton = four.current.find(
          ...
          const goodButton = four.current.find(
            (button) => button.value === currentWord.english
          );
          const badButton = four.current.find(
            (button) => button.value === value
          );
          goodButton.classList.add(classes.goodButton);
  ...
  useEffect(() => {
    ...
    const keyboardClick = (event) => {
      ...
      let elem;
      if (
        event.keyCode === keyCodeArray.top1 ||
        event.keyCode === keyCodeArray.num1
      ) {
        elem = four.current[0];
      } else if (
        event.keyCode === keyCodeArray.top2 ||
        event.keyCode === keyCodeArray.num2
      ) {
        elem = four.current[1];
      } else if (
        event.keyCode === keyCodeArray.top3 ||
        event.keyCode === keyCodeArray.num3
      ) {
        elem = four.current[2];
      } else if (
        event.keyCode === keyCodeArray.top4 ||
        event.keyCode === keyCodeArray.num4
      ) {
        elem = four.current[3];
      }
      ...
      {endGame ? (
        ...
        <div ref={gameBoard} className={classes.gameContainer}>
          ...
          <Transition
            in={!block}
            timeout={5000}
            onEntered={(event) => answer(event.dataset.name, false)}
          >
            {(state) => (
              <Typography
          ...
          <div className={classes.contentWrap}>
            ...
            >{`Правильные ответы: ${correctAnswers.length || 0}`}</Typography>
            <Typography
              color="secondary"
              variant="subtitle1"
              className={classes.fail}
            >{`Ошибки: ${failAnswers.length || 0}`}</Typography>
          </div>
        </div>
      ) : (
